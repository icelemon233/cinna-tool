...
async function writeClipboardText(text: string): Promise<void> {
  if (window.cinnaAPI?.writeClipboardText) {
    await window.cinnaAPI.writeClipboardText(text);
    return;
  }
...
const ClipboardPage: React.FC<ClipboardPageProps> = ({ mode = 'main' }) => {
  ...
  const handleReadClipboard = async () => {
    try {
      const text = window.cinnaAPI?.readClipboardText
        ? await window.cinnaAPI.readClipboardText()
  ...
  const handleOpenFloating = async () => {
    try {
      await window.cinnaAPI?.openClipboardFloatingWindow();
    } catch {
