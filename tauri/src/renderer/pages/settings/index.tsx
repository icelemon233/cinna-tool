...
function getDesktopRuntimeVersion(): string {
  const bridgeVersion = window.cinnaAPI?.version;
...
const SettingsPage: React.FC<SettingsPageProps> = ({ activeSection, onSectionChange }) => {
  ...
  useEffect(() => {
    if (!window.cinnaAPI?.storeGet) return;

    window.cinnaAPI.storeGet('app-preferences').then((stored) => {
  ...
  const appInfo = useMemo(
    () => [
      ...
      ['Runtime', getDesktopRuntimeVersion()],
      ['Developer', '🧊🍋 icelemon'],
      ['App', 'CinnaTool'],
    ],
    [t]
  ...
  const handleHardwareChange = async (checked: boolean) => {
    ...
    await window.cinnaAPI?.storeSet?.('app-preferences', { hardwareAcceleration: checked });
    message.info(t('settings.restartRequired'));
  };

  const handleDownloadFolder = async () => {
    const folder = await window.cinnaAPI?.selectDownloadFolder?.();
    if (folder) {
  ...
  const handleWallpaperFile = async (kind: WallpaperKind): Promise<WallpaperPickerResult> => {
    const selectFile = window.cinnaAPI?.selectWallpaperFile;
    if (!selectFile) {
      return 'failed';
    }
  ...
  const handleLocalWallpaperFile = async (kind: WallpaperKind, file: File | undefined) => {
    ...
    if (filePath && window.cinnaAPI?.resolveWallpaperFile) {
      try {
        const resolvedFile = await window.cinnaAPI.resolveWallpaperFile(filePath, kind);
        if (resolvedFile) {
        ...
        <div className="settings-content-inner">
          ...
          {activeSection === 'appearance' && (
            <AppearanceSection
              ...
              language={language}
              nativeWallpaperPickerAvailable={Boolean(window.cinnaAPI?.selectWallpaperFile)}
