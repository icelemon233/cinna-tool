...
function getQuickActionFromKeyboardEvent(event: KeyboardEvent): QuickAction | null {
  if (isEditableTarget(event.target)) return null;
  if ((!event.metaKey && !event.ctrlKey) || event.altKey || event.shiftKey) return null;

  // Cmd/Ctrl+0 is handled by Tauri's global shortcut even while this window is focused.
  switch (event.key) {
    case '1':
      return 'create-todo';
    case '2':
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isClipboardFloatingWindow || isTranslationQuickWindow) return undefined;

    const timerId = window.setTimeout(() => {
      window.cinnaAPI?.notifyShellReady?.();
    }, 0);
  ...
  useEffect(() => {
    ...
    window.cinnaAPI?.setAppLocale(language).catch(() => {});
  }, [language]);
  ...
  useEffect(() => {
    return window.cinnaAPI?.onShowClipboardPage?.(() => {
      setPage('clipboard');
    });
  }, []);

  useEffect(() => {
    return window.cinnaAPI?.onOpenAISettings?.(() => {
  ...
  useEffect(() => {
    const handleQuickAction = (action: QuickAction) => {
      switch (action) {
        ...
        case 'toggle-floating':
          void window.cinnaAPI?.toggleClipboardFloatingWindow?.();
          break;
        default:
          break;
      }
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      const action = getQuickActionFromKeyboardEvent(event);
      if (!action) return;
    ...
    const unlistenQuickAction = window.cinnaAPI?.onQuickAction?.(handleQuickAction);
  ...
  useEffect(() => {
    if (!wallpaperFile?.path || !window.cinnaAPI?.resolveWallpaperFile) return undefined;

    let alive = true;
    window.cinnaAPI.resolveWallpaperFile(wallpaperFile.path, 'static').then((file) => {
      if (!alive) return;
      if (!file) {
  ...
  useEffect(() => {
    if (!dynamicWallpaperFile?.path || !window.cinnaAPI?.resolveWallpaperFile) return undefined;

    let alive = true;
    window.cinnaAPI.resolveWallpaperFile(dynamicWallpaperFile.path, 'dynamic').then((file) => {
      if (!alive) return;
      if (!file) {
  ...
  const openQuickClipboardAdd = () => {
    ...
    const clipboardText = window.cinnaAPI?.readClipboardText?.() ?? Promise.resolve('');
    void clipboardText
      .then((text) => {
