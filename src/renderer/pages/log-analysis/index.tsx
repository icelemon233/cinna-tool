...
import ChatSettingsPanel from '@/pages/chat/components/ChatSettings';
...
const LogAnalysisPage: React.FC<LogAnalysisPageProps> = ({ active, droppedFile }) => {
  ...
  const { loadModels, loadSettings, models, settings, updateSettings } = useChatStore();
  ...
  const addAiAnalysis = useCallback(async (baseReport: LogAnalysisReport, runId: number) => {
    ...
    if (!isAiConfigured('ai')) {
      setReport((current) => ({
        ...(current ?? baseReport),
        mode: 'ai',
        aiError: 'AI 对话配置未完成，请先在 AI 聊天中配置模型、Base URL 和 API Key。',
      }));
      return;
    }
  ...
  const handleSaveAiConfig = (nextSettings: Partial<ChatSettings>) => {
    updateSettings(nextSettings);
    message.success('AI 配置已同步到 AI 聊天');
  };
