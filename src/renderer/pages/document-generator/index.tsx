...
import type { ModelInfo } from '@/shared/types/electron';
import './index.css';
...
const DocumentGeneratorPage: React.FC<DocumentGeneratorPageProps> = ({ active }) => {
  ...
  const handleCopy = async () => {
    ...
    try {
      if (window.electronAPI?.writeClipboardText) {
        await window.electronAPI.writeClipboardText(previewMarkdown);
      } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      if (!window.electronAPI?.saveGeneratedDocument) {
